...
class SAGE(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        penalty = torch.mm(torch.t(attention), attention).to(self.args.device)-torch.eye(self.args.second_dense_neurons).to(self.args.device)
...
class SEAL(torch.nn.Module):
    ...
    def _setup(self):
        ...
        """
        self.graph_level_model = SAGE(self.args, self.number_of_features).to(self.args.device)
